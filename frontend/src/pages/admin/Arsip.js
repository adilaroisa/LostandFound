...
import { EyeIcon, TrashIcon, CubeIcon, UserIcon } from '@heroicons/react/24/solid';
...
const Arsip = () => {
    ...
    const handleDelete = async (id) => {
        ...
        const result = await Swal.fire({
            ...
            text: "Data arsip ini akan dihapus dan tidak bisa dikembalikan!",
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Terhapus!', 'Data arsip berhasil dihapus permanen.', 'success');
            } catch (error) { 
                            ...
                            <tr key={item.id} style={{opacity: 0.9, backgroundColor: item.status === 'rejected' ? '#fff1f2' : '#f0f9ff'}}>
                                ...
                                <td className="col-action">
                                    <div className="action-buttons">
                                        <button className="btn btn-detail" onClick={() => setSelectedReport(item)} title="Lihat Detail" style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <EyeIcon style={{width:'16px'}}/> Detail
                                        </button>
                                        <button className="btn btn-delete" onClick={() => handleDelete(item.id)} title="Hapus Permanen" style={{display:'flex', alignItems:'center', gap:'5px'}}>
                ...
                <div className="modal-overlay" onClick={() => setSelectedReport(null)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-footer">
                            <button className="btn btn-delete" onClick={() => handleDelete(selectedReport.id)} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                <TrashIcon style={{width:'18px'}}/> Hapus Permanen
                            </button>
                        </div>
                    </div>
                </div>
