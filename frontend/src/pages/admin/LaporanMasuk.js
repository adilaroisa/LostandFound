...
import { EyeIcon, CheckIcon, XMarkIcon, CubeIcon, UserIcon, PlusIcon } from '@heroicons/react/24/solid';
...
const LaporanMasuk = () => {
    ...
    const handleSave = async (e) => {
        ...
        // SweetAlert untuk simpan laporan
        const result = await Swal.fire({
            title: 'Simpan Laporan?',
            text: "Pastikan data barang dan pelapor sudah benar.",
            icon: 'question',
    ...
    const handleAction = async (id, action) => {
        // Dinamis SweetAlert config berdasarkan aksi
        let swalTitle, swalText, swalIcon, swalConfirmColor, swalConfirmText;

        if (action === 'approved') {
            swalTitle = "Terima Laporan?";
            swalText = "Laporan ini akan disetujui dan tampil di daftar barang publik.";
            swalIcon = "question";
            swalConfirmColor = "#065F46";
            swalConfirmText = "Ya, Terima Laporan";
        } else if (action === 'rejected') {
            swalTitle = "Tolak Laporan?";
            swalText = "Laporan ini akan ditolak dan dipindahkan ke menu Arsip.";
            swalIcon = "warning";
            swalConfirmColor = "#d33";
            swalConfirmText = "Ya, Tolak Laporan";
        }
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Berhasil!', 'Aksi telah berhasil diproses.', 'success');
            } catch (error) { 
        ...
        <div className="table-card">
            <div className="table-header-action" style={{flexDirection: 'column', alignItems: 'stretch', gap: '15px'}}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                    ...
                    <button className="btn btn-approve" onClick={handleAddNew} style={{padding:'8px 16px', fontSize:'0.9rem', display:'flex', alignItems:'center', gap:'5px'}}>
                        <PlusIcon style={{width:'18px'}}/> Buat Laporan
                    </button>
                </div>
                            ...
                            <tr key={item.id}>
                                ...
                                <td className="col-action">
                                    <div className="action-buttons">
                                        <button className="btn btn-detail" onClick={() => handleDetail(item)} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <EyeIcon style={{width:'16px'}}/> Lihat Detail
                                        </button>
                                        <button className="btn btn-approve" onClick={() => handleAction(item.id, 'approved')} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <CheckIcon style={{width:'16px'}}/> Terima
                                        </button>
                                        <button className="btn btn-reject" onClick={() => handleAction(item.id, 'rejected')} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <XMarkIcon style={{width:'16px'}}/> Tolak
                                        </button>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{maxWidth: modalMode === 'detail' ? '800px' : '700px'}}>
                                ...
                                <form onSubmit={handleSave}>
                                    ...
                                    <div className="modal-footer" style={{padding:'0', marginTop:'20px', background:'none', border:'none'}}>
                                        ...
                                        <button type="submit" className="btn btn-approve" style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <CheckIcon style={{width:'18px'}}/> Simpan Laporan
                                        </button>
                                    </div>
                                </form>
                            )}
                        ...
                        {modalMode === 'detail' && selectedReport && (
                            <div className="modal-footer">
                                <button className="btn btn-approve" onClick={() => handleAction(selectedReport.id, 'approved')} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                    <CheckIcon style={{width:'18px'}}/> Terima
                                </button>
                                <button className="btn btn-reject" onClick={() => handleAction(selectedReport.id, 'rejected')} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                    <XMarkIcon style={{width:'18px'}}/> Tolak
                                </button>
                            </div>
                        )}
                    </div>
