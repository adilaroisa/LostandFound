...
import { EyeIcon, PencilSquareIcon, CheckCircleIcon, TrashIcon, CubeIcon, UserIcon } from '@heroicons/react/24/solid';
...
const ActiveItems = () => {
    ...
    const handleSave = async (e) => {
        ...
        // SweetAlert untuk Edit Data
        const result = await Swal.fire({
            title: 'Simpan Perubahan?',
            text: "Data barang ini akan diperbarui.",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#F59E0B', // Warna kuning edit
    ...
    const handleAction = async (id, action) => {
        let swalTitle, swalText, swalIcon, swalConfirmColor, swalConfirmText;

        if (action === 'delete') {
            swalTitle = "Hapus Permanen?";
            swalText = "Data barang ini akan dihapus permanen dari sistem!";
            swalIcon = "warning";
            swalConfirmColor = "#d33";
            swalConfirmText = "Ya, Hapus!";
        } else if (action === 'resolved') {
            swalTitle = "Tandai Kasus Selesai?";
            swalText = "Barang ini sudah dikembalikan ke pemiliknya? Data akan dipindah ke Arsip.";
            swalIcon = "success"; // icon hijau centang
            swalConfirmColor = "#065F46";
            swalConfirmText = "Ya, Selesai!";
        }
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Berhasil!', action === 'delete' ? 'Data berhasil dihapus.' : 'Laporan telah diselesaikan.', 'success');
            } catch (error) { 
                            ...
                            <tr key={item.id}>
                                ...
                                <td className="col-action">
                                    <div className="action-buttons">
                                        <button className="btn btn-detail" onClick={() => handleDetail(item)} title="Lihat Detail" style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <EyeIcon style={{width:'16px'}}/> Detail
                                        </button>
                                        <button className="btn btn-detail" onClick={() => handleEdit(item)} title="Edit" style={{backgroundColor:'#F59E0B', display:'flex', alignItems:'center', gap:'5px'}}>
                                            <PencilSquareIcon style={{width:'16px'}}/> Edit
                                        </button>
                                        <button className="btn btn-approve" onClick={() => handleAction(item.id, 'resolved')} title="Selesai" style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <CheckCircleIcon style={{width:'16px'}}/> Selesai
                                        </button>
                                        <button className="btn btn-delete" onClick={() => handleAction(item.id, 'delete')} title="Hapus" style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <TrashIcon style={{width:'16px'}}/> Hapus
                                        </button>
                                    </div>
                                </td>
                            </tr>
                    ...
                    <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{maxWidth: modalMode === 'detail' ? '800px' : '700px'}}>
                        ...
                        <div className="modal-body">
                                ...
                                <div className="detail-container">
                                    ...
                                    <div className="detail-right">
                                        <div className="detail-section-title" style={{display:'flex', alignItems:'center', gap:'8px'}}>
                                            <CubeIcon style={{width:'20px', color:'#475569'}}/> Informasi Barang
                                        </div>
                                        ...
                                        <div className="detail-section-title" style={{marginTop:'20px', display:'flex', alignItems:'center', gap:'8px'}}>
                                            <UserIcon style={{width:'20px', color:'#475569'}}/> Informasi Pelapor
                                        </div>
                            ...
                            {modalMode === 'edit' && (
                                <form onSubmit={handleSave}>
                                    <div className="detail-section-title" style={{display:'flex', alignItems:'center', gap:'8px'}}>
                                        <CubeIcon style={{width:'20px', color:'#475569'}}/> Data Barang
                                    </div>
                                    ...
                                    <div className="detail-section-title" style={{marginTop:'25px', display:'flex', alignItems:'center', gap:'8px'}}>
                                        <UserIcon style={{width:'20px', color:'#475569'}}/> Data Pelapor
                                    </div>
                                    <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px', marginBottom:'15px'}}>
                                    ...
                                    <div className="modal-footer" style={{padding:'0', marginTop:'20px', background:'none', border:'none'}}>
                                        ...
                                        <button type="submit" className="btn btn-approve" style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            <PencilSquareIcon style={{width:'18px'}}/> Update
                                        </button>
                                    </div>
                                </form>
                            )}
                        ...
                        {modalMode === 'detail' && selectedItem && (
                            <div className="modal-footer">
                                <button className="btn btn-delete" onClick={() => handleAction(selectedItem.id, 'delete')} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                    <TrashIcon style={{width:'18px'}}/> Hapus Barang
                                </button>
                                <button className="btn btn-approve" onClick={() => handleAction(selectedItem.id, 'resolved')} style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                    <CheckCircleIcon style={{width:'18px'}}/> Tandai Selesai
                                </button>
                            </div>
                        )}
                    </div>
